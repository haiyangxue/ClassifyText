...
//    int IncludeChineseOrAlpha(String str)//返回0：无中文或英文字母，返回1：有中文或英文字母
//    {
//            char cc[str.length()];
//            strcpy(cc,str.c_str());
//            char *c=&cc[0];
...
//                }
//
//                if (*c==NULL) break;  //如果到字符串尾则说明该字符串没有中文字符
//            }
//            return 0;
////        char c;
////        while(1)
...
//    }
    ...
    inline int IncludeChineseOrAlpha(string str)//返回0：无中文或英文字母，返回1：有中文或英文字母
    {
        char cc[str.length()];
        strcpy(cc,str.c_str());
        char *c=&cc[0];
        while(1)
        {
            ...
            if (*c) break;  //如果到字符串尾则说明该字符串没有中文字符
        }
        return 0;
    }
    ...
    template<class T>
    void Join(T begin, T end, string &res, const string &connector,unordered_set<string> &stopword) {
        if (begin == end) {
            return;
        }
        stringstream ss;
    ...
    template<class T>
    string Join(T begin, T end, const string &connector,unordered_set<string> &stopword) {
        string res;
        Join(begin, end, res, connector, stopword);
        return res;
    }

    template<class T>
    vector<string> Seg(T begin, T end, unordered_set<string> &stopword) {
        vector<string> res;
        if (begin == end) {
            return res;
        }
        ...
        while (begin != end) {
            ...
            if (stopword.count(*begin)==0 && IncludeChineseOrAlpha(*begin)) {
                res.push_back(*begin);
            }
            begin++;
        }
